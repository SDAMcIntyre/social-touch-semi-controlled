...
if __name__ == "__main__":
    ...
    for session in sessions:
        ...
        output_filename_abs = os.path.join(db_path_output, session + "_kinect_arm_roi_metadata.txt")
        ...
        for mkv_filename_abs, mkv_filename in zip(mkv_files_abs, mkv_files):
            ...
            arm_roi = KinectRegionOfInterest(mkv_filename_abs, output_filename_abs)

            if not force_processing and arm_roi.is_already_processed():
                print(f"Results already exist and user decided to not force processing.")
                print(f"Go to next file --->")
            else:
                arm_roi.initialise_video()
                frame_number = arm_roi.select_good_frame()
