...
if __name__ == "__main__":
    ...
    show_endresults = False  # If user wants to monitor what's happening
    ...
    db_path_output = os.path.join(db_path, "3_merged", "1_kinect_and_nerve_shandata", "2_by-trials_single-touches-index")
    output_figure_path = os.path.join(path_tools.get_result_path(), "semi-controlled", "kinect_and_nerve_shandata",
                                      "0_by-trials", "trials_overview")
    if not os.path.exists(db_path_output) and save_results:
        os.makedirs(db_path_output)
    if not os.path.exists(output_figure_path) and save_figures:
    ...
    for session in sessions:
        ...
        for data_filename_abs, data_filename in zip(files_abs, files):
            ...
            scd.set_variables(dropna=False, ignorestimulus=True)
            ...
            if save_results:
                ...
                if not force_processing and os.path.exists(filename_output_abs):
                    ...
                    with open(path_tools.winapi_path(filename_output_abs), 'w') as f:
