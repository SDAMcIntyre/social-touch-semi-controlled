...
from .touch_population_data import PopulationData
...
def diagnose_population_data(
    ...
    """2×2 diagnostic figure for the loaded PopulationData.
    ...
    all_contact_xyz = fv[cp_idx]               # (C, 3)

    spike_touch_indices = np.where(spike_mask)[0]
    spike_cp_mask = np.isin(cp_touch, spike_touch_indices)
    spike_contact_xyz = fv[cp_idx[spike_cp_mask]]  # contacts from spike touches

    T = len(gesture_types)
    n_spike_touches = int(spike_mask.sum())
    unique_spike_vtx = int(np.unique(cp_idx[spike_cp_mask]).shape[0]) if spike_cp_mask.any() else 0
    ...
    # --- top-left: XY view ---
    ax = axes[0, 0]
    ax.scatter(all_contact_xyz[:, 0], all_contact_xyz[:, 1],
               c="steelblue", alpha=0.3, s=2, linewidths=0, label="all contacts")
    if len(spike_contact_xyz) > 0:
        ax.scatter(spike_contact_xyz[:, 0], spike_contact_xyz[:, 1],
                   c="red", alpha=0.7, s=4, linewidths=0, label="spike contacts")
    ax.set_xlabel("X (mm)", color="white")
    ax.set_ylabel("Y (mm)", color="white")
    ax.set_title("XY view — all contacts + spike contacts", color="white")
    ax.legend(facecolor="#2a2a2a", labelcolor="white", markerscale=2)

    # --- top-right: XZ view ---
    ax = axes[0, 1]
    ax.scatter(all_contact_xyz[:, 0], all_contact_xyz[:, 2],
               c="steelblue", alpha=0.3, s=2, linewidths=0, label="all contacts")
    if len(spike_contact_xyz) > 0:
        ax.scatter(spike_contact_xyz[:, 0], spike_contact_xyz[:, 2],
                   c="red", alpha=0.7, s=4, linewidths=0, label="spike contacts")
    ax.set_xlabel("X (mm)", color="white")
    ax.set_ylabel("Z (mm)", color="white")
    ax.set_title("XZ view — all contacts + spike contacts", color="white")
    ax.legend(facecolor="#2a2a2a", labelcolor="white", markerscale=2)

    # --- bottom-left: histogram of contact points per touch ---
    ax = axes[1, 0]
    cp_counts_per_touch = np.bincount(cp_touch, minlength=T)
    all_counts = cp_counts_per_touch
    spike_counts = cp_counts_per_touch[spike_mask]
    bin_max = int(all_counts.max()) if len(all_counts) > 0 else 1
    bins = np.arange(0, bin_max + 2)
    ax.hist(all_counts, bins=bins, color="steelblue", alpha=0.7, label="all touches")
    if len(spike_counts) > 0:
        ax.hist(spike_counts, bins=bins, color="red", alpha=0.7, label="spike touches")
    ax.set_xlabel("Contact points per touch", color="white")
    ax.set_ylabel("Count", color="white")
    ax.set_title("Contact points per touch distribution", color="white")
...
def diagnose_spike_extraction(
    ...
    """1×3 diagnostic figure for spike vertex extraction.
    ...
    # --- left: XY view ---
    ax = axes[0]
    ax.scatter(forearm_vertices[:, 0], forearm_vertices[:, 1],
    ...
    ax.set_xlabel("X (mm)", color="white")
    ax.set_ylabel("Y (mm)", color="white")
    ax.set_title("XY view", color="white")
    ax.legend(facecolor="#2a2a2a", labelcolor="white", markerscale=1.5)

    # --- center: XZ view ---
    ax = axes[1]
    ax.scatter(forearm_vertices[:, 0], forearm_vertices[:, 2],
               c="#555555", alpha=0.2, s=1, linewidths=0, label="forearm")
    if len(spike_xyz) > 0:
        ax.scatter(spike_xyz[:, 0], spike_xyz[:, 2],
                   c="red", alpha=0.8, s=sizes, linewidths=0, label="spike vertices")
    ax.set_xlabel("X (mm)", color="white")
    ax.set_ylabel("Z (mm)", color="white")
    ax.set_title("XZ view", color="white")
    ax.legend(facecolor="#2a2a2a", labelcolor="white", markerscale=1.5)
...
def diagnose_aggregation(
    ...
    """2×2 diagnostic figure for spike count aggregation.
    ...
    # --- top-left: XY colored by spike_count ---
    ax = axes[0, 0]
    if has_spikes:
        sc_plot = ax.scatter(
            spike_counts_df["x"], spike_counts_df["y"],
    ...
    ax.set_xlabel("X (mm)", color="white")
    ax.set_ylabel("Y (mm)", color="white")
    ax.set_title("XY — spike count per vertex", color="white")

    # --- top-right: XZ colored by spike_count ---
    ax = axes[0, 1]
    if has_spikes:
        sc_plot = ax.scatter(
            spike_counts_df["x"], spike_counts_df["z"],
            c=spike_counts_df["spike_count"], cmap=_CMAP, norm=norm,
            s=8, alpha=0.9, linewidths=0,
        )
        plt.colorbar(sc_plot, ax=ax, label="spike_count").ax.yaxis.label.set_color("white")
    ...
    ax.set_xlabel("X (mm)", color="white")
    ax.set_ylabel("Z (mm)", color="white")
    ax.set_title("XZ — spike count per vertex", color="white")

    # --- bottom-left: centroid comparison ---
    ax = axes[1, 0]
    ax.scatter(forearm_vertices[:, 0], forearm_vertices[:, 1],
    ...
    ax.set_xlabel("X (mm)", color="white")
    ax.set_ylabel("Y (mm)", color="white")
    ax.set_title("XY — centroid comparison", color="white")
    ax.legend(facecolor="#2a2a2a", labelcolor="white", markerscale=0.8)

    # --- bottom-right: spike_count distribution ---
    ax = axes[1, 1]
    if has_spikes:
...
def diagnose_projection(
    ...
    """1×3 diagnostic figure for the 2D cylindrical projection.

    Parameters
    ----------
    forearm_vertices:
        (V, 3) forearm mesh vertices (used only for count reference in metadata).
    spike_counts_df:
        DataFrame with columns x, y, z, spike_count.
    uv_all:
        (V, 2) UV coordinates for all forearm vertices.
    uv_spikes:
        (K, 2) UV coordinates for spike vertices.
    projection_metadata:
    ...
    # --- left: full UV scatter ---
    ax = axes[0]
    ax.scatter(uv_all[subsample, 0], uv_all[subsample, 1],
               c="#555555", alpha=0.2, s=1, linewidths=0, label="forearm (1:5)")
    if has_spikes:
        sp = ax.scatter(
    ...
    ax.set_title("UV — forearm + spikes", color="white")
    ax.legend(facecolor="#2a2a2a", labelcolor="white", markerscale=2)

    # --- center: spike UV only ---
    ax = axes[1]
    if has_spikes:
        sp = ax.scatter(
            uv_spikes[:, 0], uv_spikes[:, 1],
    ...
    text_lines = [
        ...
        f"Centroid (mm): {_fmt_vec(centroid)}",
        "",
        "Rotation matrix (R):",
        f"  R[0]: {_fmt_vec(R[0])}",
        f"  R[1]: {_fmt_vec(R[1])}",
    ...
    fig.suptitle("Step 4 — 2D Projection", color="white", fontsize=14, y=1.01)
    fig.tight_layout()
    return fig
...
def run_diagnostics(
    ...
    neuron_contacts_xyz:
        (M, 3) all contact vertices used for projection centroid.
    uv_all:
        (V, 2) UV for all forearm vertices.
    uv_spikes:
        (K, 2) UV for spike vertices.
    projection_metadata:
        Dict with keys: centroid, rotation_matrix, axis_direction, mean_radius.
    output_dir:
    ...
    show:
        If True, call ``plt.show()`` after each figure (blocking).
    """
    ...
    steps = [
        (
            ...
            lambda: diagnose_population_data(population_data, spike_mask),
        ),
        (
            "step2_spike_extraction.png",
            lambda: diagnose_spike_extraction(forearm_vertices, spike_vertex_indices),
        ),
        (
            "step3_aggregation.png",
            lambda: diagnose_aggregation(
                spike_counts_df, forearm_vertices, spike_xyz, neuron_contacts_xyz
            ),
        ),
        (
            "step4_projection.png",
            lambda: diagnose_projection(
                forearm_vertices, spike_counts_df, uv_all, uv_spikes, projection_metadata
            ),
        ),
    ]
    ...
    for filename, build_fig in steps:
        fig = build_fig()
        if save:
            fig.savefig(diag_dir / filename, dpi=150, bbox_inches="tight",
                        facecolor=_BG)
        if show:
            plt.show()
        plt.close(fig)
