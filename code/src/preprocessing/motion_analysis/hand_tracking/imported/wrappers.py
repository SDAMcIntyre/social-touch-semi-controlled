  ...
  def __init__(self, model_path):
    ...
    with self.graph.as_default():
      with tf.variable_scope('prior_based_hand'):
        ...
        self.uv = self.uv[0]
      model_path_abs = os.path.join(os.path.dirname(os.path.abspath(__file__)), model_path)
      tf.train.Saver().restore(self.sess, model_path_abs)
...
class ModelPipeline:
  ...
  def corrected_theta(self,corrected_xyz):
    """
    Derive joint angles from corrected joint positions by Kinect depth
    """
    delta, length = xyz_to_delta(corrected_xyz, MPIIHandJoints)
    delta *= length
    pack = np.concatenate(
      [corrected_xyz, delta, self.mpii_ref_xyz, self.mpii_ref_delta], 0
    )
    theta = self.ik_model.process(pack)
