...
class XYZVisualizationHandler:
    ...
    def create_frame(self, frame_index: int, depth_image: np.ndarray, monitoring_data: Dict[str, Any]) -> np.ndarray:
        ...
        if depth_image is not None:
            ...
            for data in monitoring_data.values():
                ...
                if not np.isnan(px) and not np.isnan(py):
                    cv2.drawMarker(jet_map, (int(round(px)), int(round(py))), (255, 255, 255), cv2.MARKER_CROSS, 20, 2)
        ...
        for name, data in monitoring_data.items():
            cv2.putText(canvas, f"- Sticker: {name}", (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            text_y += 25
