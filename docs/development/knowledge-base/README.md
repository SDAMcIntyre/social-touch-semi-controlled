...
| [Prefect removal — shared `PREFECT_HOME`, swallowed stderr, silenced root logger](note-prefect-removal.md) | A per-user `~/.prefect` DB shared with a sibling repo bricked the older Prefect at an alembic revision; `DEVNULL` turned the one-line error into a 30 s GUI freeze; `from prefect import ...` installed a root log handler that made every `basicConfig` a no-op. Why the dependency was dropped rather than pinned. | `launcher_window.py`, the 10 entry scripts in `code/scripts/` |
