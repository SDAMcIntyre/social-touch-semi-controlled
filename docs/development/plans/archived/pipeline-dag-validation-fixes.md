...
- [ ] A failure in `track_led_blinking` no longer prevents `track_stickers_raw`
      (and other independent branches) from running
- [ ] `validate_hand_extraction` executes before `generate_3d_hand_in_motion` in
      both code and DAG config
- [ ] `find_single_touches.depends_on` lists `generate_xyz_stickers` instead of
      `generate_3d_hand_in_motion`
- [ ] Manual DAG enforces `prepare_stickers_colorspace` before
      `review_handstickers_color_threshold`
- [ ] `define_trial_chunks` in manual DAG depends on `prepare_led_tracking`
- [ ] At least the top 3 validation gates check file size + basic schema (not just
      existence)
- [ ] Manual pipeline uses `TaskExecutor` for all task dispatch
- [ ] All existing tests still pass
- [ ] A new knowledge-base note documents the DAG validation ordering pattern
...
- [ ] Task 1.1 — In `preprocess_workflow_kinect_auto_dag.yaml`: change
...
- [ ] Task 2.1 — Add `get_dependents(task_name) -> set[str]` method to
      `DagConfigHandler` that walks the DAG and returns all tasks transitively
      depending on `task_name`
- [ ] Task 2.2 — In the auto pipeline's main loop, replace the "skip everything
...
- [ ] Task 2.3 — Track failed tasks in a set so `can_run()` can implicitly
...
- [ ] Task 3.1 — Import `TaskExecutor` and optionally `PipelineMonitor` in the
      manual script
- [ ] Task 3.2 — Replace each `if dag_handler.can_run(...): ... mark_completed()`
      block with a `TaskExecutor` context manager, mirroring the auto pattern
- [ ] Task 3.3 — Verify that exception suppression in `TaskExecutor.__exit__`
...
- [ ] Task 4.1 — `is_2d_stickers_tracking_valid()`: verify the JSON contains the
      expected `"validated"` key **and** that the referenced ROI CSV has >0 rows
- [ ] Task 4.2 — `is_correlation_videos_threshold_defined()`: verify the
      threshold value is a finite number, not just that the key exists
- [ ] Task 4.3 — `.SUCCESS` sentinel for curated hands: verify the curated `.pkl`
      file size is >0 and the sentinel was written after the pkl
- [ ] Task 4.4 — `_single-touches-corrected.csv` check: verify the file has a
      header row and >0 data rows
- [ ] Task 4.5 — Add a shared utility `validate_csv_not_empty(path, min_rows=1)`
...
- [ ] Task 5.2 — Add inline comments at the top of both workflow scripts
...
- [ ] `DagConfigHandler.get_dependents()` returns correct transitive closure for
      a known test DAG
- [ ] `DagConfigHandler.get_dependents()` returns empty set for a leaf task
- [ ] `validate_csv_not_empty()` rejects zero-byte, header-only, and missing files
- [ ] `validate_csv_not_empty()` accepts a file with header + 1 data row

### Integration Tests

- [ ] Simulate a `track_led_blinking` failure and verify `track_stickers_raw`
      still executes (mocked pipeline)
- [ ] Simulate a `validate_hand_extraction` failure and verify
      `generate_3d_hand_in_motion` is blocked (since validation now precedes it)

### Manual Verification

- [ ] Run the manual pipeline on one session config and confirm all `TaskExecutor`
      context managers log correctly
- [ ] Run the auto pipeline with one task forced to fail and confirm only its
      dependents are marked SKIPPED in the Excel report

### Edge Cases

- [ ] A task with no dependents fails — no other tasks should be skipped
- [ ] All tasks disabled except one leaf — pipeline runs just that task
- [ ] Stale `.SUCCESS` sentinel from a previous incompatible run — validation
...
- [ ] Create knowledge-base note: `docs/development/knowledge-base/note-dag-validation-ordering.md`
- [ ] Update `docs/development/knowledge-base/README.md` index
- [ ] Add header docstrings to both workflow scripts describing the manual-auto contract
