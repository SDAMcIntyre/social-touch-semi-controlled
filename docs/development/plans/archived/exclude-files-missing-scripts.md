...
- [x] All three scripts read `exclude_files` from the DAG config and filter `block_files` before the processing loop
- [x] Cherry-picking files in the GUI and running any of these workflows processes only the checked files
- [x] No regressions — when `exclude_files` is absent or empty, all files are processed as before
...
- [x] Task 1.1 — In `view_merged_neural_kinect.py`: add exclude_files filtering after `get_block_files()` call (~line 174)
- [x] Task 1.2 — In `preprocess_workflow_kinect_manual.py`: add exclude_files filtering after `get_block_files()` call (~line 377)
- [x] Task 1.3 — In `preprocess_workflow_kinect_visualisation.py`: add exclude_files filtering after `get_block_files()` call (~line 214)
...
### Manual Verification
- [x] Open the DAG Launcher GUI, select a workflow (e.g. View Merged Neural Kinect)
- [x] Uncheck one or more config files within the selected kinect directory
- [x] Save and Run — confirm only the checked files are processed (check console output for block names)
- [x] Repeat for Preprocess Kinect Manual and Preprocess Kinect Visualisation workflows
- [x] Confirm that with no `exclude_files` in the YAML (or an empty list), all files are processed as before

### Edge Cases
- [x] `exclude_files` key absent from YAML — all files processed (no crash)
- [x] `exclude_files` key present but empty list — all files processed
- [x] All files excluded — no files processed, no crash (graceful empty loop)

---

## Documentation Plan

- [x] No documentation changes needed (internal bug fix aligning with existing feature)
