...
- [ ] `ForearmParameters` has no `frame_id` property — accessing it raises `AttributeError`
- [ ] `ForearmFrameParametersFileHandler.load()` requires both `"frame_ids"` and `"representative_frame_id"` keys
- [ ] `is_valid_structure()` rejects JSON entries with only `"frame_id"` (old format)
- [ ] `extract_forearm()` uses `FrameDepthAverager.average()` for all captures (no `is_averaged` branching)
- [ ] Single-frame extraction produces identical output PLY to the pre-change version
- [ ] All 7 former `frame_id` call sites compile and run correctly with `representative_frame_id`
- [ ] No remaining references to the string `".frame_id"` in the forearm extraction package (except unrelated sticker modules)
...
- [ ] 1.1 — Delete the `frame_id` property (lines 35-38) from `forearm_parameters.py`
- [ ] 1.2 — Update `forearm_catalog.py` (5 sites):
...
- [ ] 1.3 — Update `extract_participant_forearm.py` line 231: `mkv[video_config.frame_id]` -> `mkv[video_config.representative_frame_id]`
- [ ] 1.4 — Update `preprocess_pipeline_extract_forearm_manual.py` line 60: `f"frame {p.frame_id}"` -> `f"frame {p.representative_frame_id}"`
...
- [ ] 2.1 — Simplify `load()` (lines 65-75): replace three-path branching with direct reads of `frame_ids` and `representative_frame_id`. Keep the guard that resets `representative_frame_id` to `min(frame_ids)` if not a member (data-integrity check, not backward compat).
- [ ] 2.2 — Simplify `is_valid_structure()` (lines 128-140): require `"frame_ids"` (non-empty list) and `"representative_frame_id"` (int). Remove the `elif "frame_id" in item` branch.
...
### Manual Verification
- [ ] Run the forearm extraction pipeline on a single-frame capture. Compare output PLY to pre-change output (should be identical).
- [ ] Run the forearm extraction pipeline on a multi-frame averaged capture. Verify identical results.
- [ ] Launch `neural_kinect_scene_viewer.py` — confirm forearm point clouds/meshes load correctly in the 3D viewer.
- [ ] Verify `ForearmFrameParametersFileHandler.load()` successfully loads an existing new-format JSON file.

### Edge Cases
- [ ] JSON file with `representative_frame_id` not in `frame_ids` — guard should still reset it to `min(frame_ids)`
- [ ] Empty JSON file (empty list) — `is_valid_structure()` should return `True`
- [ ] Single-frame capture `frame_ids=[N]` — `FrameDepthAverager` processes it without error
...
- [ ] No new user guide needed — this is an internal cleanup
- [ ] Update the completed forearm-frame-averaging plan with a note referencing this cleanup
