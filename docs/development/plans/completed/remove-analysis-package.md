...
Added:
- `code/src/postprocessing/receptive_field/__init__.py`
- `code/src/postprocessing/receptive_field/rf_clustering.py` (detected by git as a
  58 %-similarity rename of `code/src/analysis/receptive_field_mapping/engine.py`)
- `docs/changelogs/remove-analysis-package.md`
- `docs/development/plans/active/remove-analysis-package.md`
...
- `code/src/utils/gui/dag_launcher/grid_group_dialog.py`
...
Deleted: 165 tracked files —
`code/src/analysis/**` (140), `code/tests/test_{rf_*,gmm_clusterer,iff_windowed_mean,forearm_slim_uv,feature_space_renderer,fit_models}.py` (12),
`code/scripts/{analysis_workflow_*,sandbox_gradient_ridge_boundary,migrate_output_dirs,inspect_touch_feature_ranges,diagnose_*,flatten_forearm_sandbox,compare_flattening_methods}.py` (10),
`configs/analyse_workflow_*.yaml` (3), plus
`code/src/utils/gui/dag_launcher/feature_combination_dialog.py`.
Also removed from disk: 11 gitignored files (9 under `code/src/analysis/`, 2 `analyse_*.layout.json`).

Edited but **not committable** (gitignored at `.gitignore:147`):
- `CLAUDE.md`
